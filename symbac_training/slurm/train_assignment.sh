#!/usr/bin/env bash
# SLURM job — train AssignmentMLP only on sciCORE.
# Submit from inside symbac_training/:
#     cd ~/SyMBac_2/symbac_training
#     sbatch slurm/train_assignment.sh

# ── SLURM directives ───────────────────────────────────────────────────────────
#SBATCH --job-name=symbac_assignment
#SBATCH --output=logs/train_assignment.out
#SBATCH --error=logs/train_assignment.err
#SBATCH --time=2:00:00
#SBATCH --mem=16G
...
echo ""
echo "==> AssignmentMLP saved to ${OUTPUT_DIR}"
ls -lh "${OUTPUT_DIR}"
