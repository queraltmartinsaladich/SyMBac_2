...
def main():
    ...
    fig, axes = plt.subplots(
        N_MOVIES, N_FRAMES * 2,
        figsize=(N_FRAMES * 2 * 2.6, N_MOVIES * 2.8 + 0.6),
    ...
    caption = (
        "Physics-based synthetic E. coli movies generated by SyMBac² (Oshardo et al., 2023). "
        "Each row shows one movie (sparse / medium / dense by median cell count). "
        "Left panel of each pair: simulated brightfield image. Right panel: ground-truth instance "
        "segmentation mask with per-cell colour labels. Yellow arrows mark division events (parent → daughters). "
